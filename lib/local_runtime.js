...
const DEFAULT_PUN_FILE = "puns.txt";

const CONFIG_FILE = path.join(PROJECT_ROOT, "config.json");
const STATE_FILE = path.join(PROJECT_ROOT, "state.json");
const HISTORY_FILE = path.join(PROJECT_ROOT, "history.jsonl");
const AVATAR_DIR = path.join(PROJECT_ROOT, "downloaded_avatars");
...
function resolvePunFilePath(value) {
  const punFile = String(value ?? DEFAULT_PUN_FILE).trim() || DEFAULT_PUN_FILE;
  if (path.isAbsolute(punFile)) {
    return punFile;
  }
  return path.join(PROJECT_ROOT, punFile);
}
